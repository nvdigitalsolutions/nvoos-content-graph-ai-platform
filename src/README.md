...
- **Downstream collaborators:** All subdirectories — `Admin/`, future `Agents/`, `Skills/`, `A2A/`, `Federation/`, etc.; the ported waves `Workflows/`, `Queues/`, `Approvals/`, `Rest/`, `Tenant/`, `Integrations/`, `Google/`, `ContentAssistant/`, `ConversationImport/` are wired by `Plugin::register*()` methods.
