<?php
/**
 * Blueprints admin — dual registration in Platform + Content Graph.
 *
 * @since 1.0.0
...
final class BlueprintAdmin {
	...
	public function registerPlatformSections(): void {
		\NvoosContentGraphAiPlatform\Admin\PlatformSettingsRegistry::register_tab(
			'blueprints',
			__( 'Blueprints', 'nvoos-content-graph-ai-platform' )
		);

		if ( class_exists( 'NvoosContentGraphAiPlatform\Blueprints\Admin\BlueprintsDashboardSection' ) ) {
	...
	public function registerContentGraphSections(): void {
		ContentGraphRegistry::register_tab( 'blueprints', __( 'Blueprints', 'nvoos-content-graph-ai-platform' ) );
	}
}
