<?php
/**
 * Team settings page (Wave E-UI-3, sub-cluster 2).
 *
 * Aligned port of the base plugin's
 * `WP_MCP_AI_Admin_Team_Settings`
 * (`includes/admin/class-wp-mcp-ai-admin-team-settings.php`):
 * byte-identical page surface — the `wp-mcp-ai-team-settings` page
 * slug (position 25 under the team CPT menu, `manage_options`), the
 * four registered settings
 * (`wp_mcp_ai_team_default_provider|_model|_temperature|_driver_assistant`
 * with their sanitize callbacks), the tabbed render (overview /
 * configuration / available-tools / help), the configuration form
 * (provider select, model + temperature inputs, settings-hierarchy
 * card), the tools list + orchestration card, the help/quick-start +
 * support cards, the inline stylesheet, and the nonce-gated save flow
 * with the temperature clamp (0–1) and settings-updated redirect.
 *
...
class TeamSettings {
	 ...
	 * @var string
	 */
	const PAGE_SLUG = 'wp-mcp-ai-team-settings';
	...
	public function register_submenu_page(): void {
		$this->page_hook = \add_submenu_page(
			...
			__( 'Team Settings', 'nvoos-content-graph-ai-platform' ),
	...
	public function render_page(): void {
		...
		<div class="wrap">
			<h1>
				...
				<?php \esc_html_e( 'Team Settings', 'nvoos-content-graph-ai-platform' ); ?>
			</h1>
	...
	protected function render_overview_tab(): void {
		?>
		<div class="team-card">
			<h2><?php \esc_html_e( 'AI Teams Overview', 'nvoos-content-graph-ai-platform' ); ?></h2>

			<div class="team-description">
				<p><?php \esc_html_e( 'AI Teams allow you to orchestrate multiple AI professions to work together on complex tasks. Teams can coordinate workflows, delegate tasks, and combine expertise from multiple specialized AI agents.', 'nvoos-content-graph-ai-platform' ); ?></p>
			</div>

			<h3><?php \esc_html_e( 'Key Features', 'nvoos-content-graph-ai-platform' ); ?></h3>
			<ul>
				<li><?php \esc_html_e( 'Multi-Agent Coordination: Orchestrate multiple AI professions in a single workflow', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><?php \esc_html_e( 'Driver Agent: Coordinate team activities with a designated driver assistant', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><?php \esc_html_e( 'Task Delegation: Automatically route tasks to the best-suited team members', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><?php \esc_html_e( 'Shared Context: Team members share information and collaborate on solutions', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><?php \esc_html_e( 'Configuration Cascade: Team-level settings cascade to all members', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><?php \esc_html_e( 'Flexible Composition: Mix and match professions to create custom teams', 'nvoos-content-graph-ai-platform' ); ?></li>
			</ul>

			<h3><?php \esc_html_e( 'Use Cases', 'nvoos-content-graph-ai-platform' ); ?></h3>
			<ul>
				<li><strong><?php \esc_html_e( 'Content Production:', 'nvoos-content-graph-ai-platform' ); ?></strong> <?php \esc_html_e( 'Writer + Editor + SEO Specialist + Designer', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><strong><?php \esc_html_e( 'Web Development:', 'nvoos-content-graph-ai-platform' ); ?></strong> <?php \esc_html_e( 'Developer + Designer + QA Tester + Project Manager', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><strong><?php \esc_html_e( 'E-commerce:', 'nvoos-content-graph-ai-platform' ); ?></strong> <?php \esc_html_e( 'Product Manager + Copywriter + Photographer + SEO', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><strong><?php \esc_html_e( 'Marketing:', 'nvoos-content-graph-ai-platform' ); ?></strong> <?php \esc_html_e( 'Marketer + Content Creator + Social Media Manager + Analyst', 'nvoos-content-graph-ai-platform' ); ?></li>
			</ul>
		</div>
		<?php
	}
	...
	protected function render_configuration_tab(): void {
		...
		<div class="team-card">
			...
			<p class="description">
				<?php \esc_html_e( 'Configure global default settings for all AI Teams. These settings provide baseline values that cascade to all team members, but can be overridden at the individual team level via metaboxes.', 'nvoos-content-graph-ai-platform' ); ?>
			</p>
					...
					<tbody>
						<tr>
							...
							<td>
								...
								<p class="description">
									<?php \esc_html_e( 'Default AI provider for all team members. Individual teams can override this in their metabox settings.', 'nvoos-content-graph-ai-platform' ); ?>
								</p>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="text" name="wp_mcp_ai_team_default_model" id="wp_mcp_ai_team_default_model" value="<?php echo \esc_attr( $default_model ); ?>" class="regular-text" placeholder="<?php \esc_attr_e( 'e.g., gpt-4.1, claude-sonnet-5', 'nvoos-content-graph-ai-platform' ); ?>">
								<p class="description">
									<?php \esc_html_e( 'Default AI model for all team members. Leave empty to use the provider\'s default model. Individual teams can override this setting.', 'nvoos-content-graph-ai-platform' ); ?>
								</p>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="number" name="wp_mcp_ai_team_default_temperature" id="wp_mcp_ai_team_default_temperature" value="<?php echo \esc_attr( (string) $default_temperature ); ?>" class="small-text" min="0" max="1" step="0.1">
								<p class="description">
									<?php \esc_html_e( 'Default creativity/randomness setting (0.0 = deterministic, 1.0 = creative). Individual teams can override this setting.', 'nvoos-content-graph-ai-platform' ); ?>
								</p>
							</td>
						</tr>
					</tbody>
		...
		<div class="team-card">
			...
			<ol>
				<li><strong><?php \esc_html_e( 'Individual Team Settings', 'nvoos-content-graph-ai-platform' ); ?></strong> - <?php \esc_html_e( 'Configured in each team\'s metabox (highest priority)', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><strong><?php \esc_html_e( 'Team Global Defaults', 'nvoos-content-graph-ai-platform' ); ?></strong> - <?php \esc_html_e( 'This page (medium priority)', 'nvoos-content-graph-ai-platform' ); ?></li>
	...
	protected function render_tools_tab(): void {
		...
		<div class="team-card">
			...
			<p class="description">
				...
				\printf(
					...
					\esc_html__( 'Teams can access all %d core tools, and each team member (profession) may have additional specialized tools.', 'nvoos-content-graph-ai-platform' ),
					\count( $tools )
				);
				?>
			</p>
		...
		<div class="team-card">
			<h2><?php \esc_html_e( 'Team Orchestration', 'nvoos-content-graph-ai-platform' ); ?></h2>
			<p><?php \esc_html_e( 'Teams use a driver agent to coordinate activities and delegate tasks to the appropriate team members.', 'nvoos-content-graph-ai-platform' ); ?></p>
			<p><?php \esc_html_e( 'Each team member can use their profession-specific tools in addition to the core tools listed above.', 'nvoos-content-graph-ai-platform' ); ?></p>
		</div>
		<?php
	}
	...
	protected function render_help_tab( $post_type ): void {
		...
		<div class="team-card">
			...
			<ol>
				<li><strong><?php \esc_html_e( 'Create a Team:', 'nvoos-content-graph-ai-platform' ); ?></strong> <?php \esc_html_e( 'Use the "Create New Team" button to add a new AI team', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><strong><?php \esc_html_e( 'Add Members:', 'nvoos-content-graph-ai-platform' ); ?></strong> <?php \esc_html_e( 'Select professions to add as team members', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><strong><?php \esc_html_e( 'Configure Driver:', 'nvoos-content-graph-ai-platform' ); ?></strong> <?php \esc_html_e( 'Select a driver assistant to coordinate the team', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><strong><?php \esc_html_e( 'Set Team Goals:', 'nvoos-content-graph-ai-platform' ); ?></strong> <?php \esc_html_e( 'Define the team\'s purpose and workflow', 'nvoos-content-graph-ai-platform' ); ?></li>
				<li><strong><?php \esc_html_e( 'Test:', 'nvoos-content-graph-ai-platform' ); ?></strong> <?php \esc_html_e( 'Use the Test Team page to verify team coordination', 'nvoos-content-graph-ai-platform' ); ?></li>
			</ol>
		</div>
		...
		<div class="team-card">
			...
			<p>
				...
				<a href="<?php echo \esc_url( \admin_url( 'post-new.php?post_type=' . $post_type ) ); ?>" class="button button-primary">
					<?php \esc_html_e( 'Create New Team', 'nvoos-content-graph-ai-platform' ); ?>
				</a>
	...
	protected function save_settings(): void {
		...
		\wp_safe_redirect( \add_query_arg( 'settings-updated', 'true', \admin_url( 'edit.php?post_type=' . self::team_post_type() . '&page=' . self::PAGE_SLUG ) ) );
		exit;
	}
}
