<?php
/**
 * Team pages ported-class tests (Wave E-UI-3, sub-cluster 2).
 *
 * Verifies the extraction ports of the base plugin's
 * `WP_MCP_AI_Admin_Team_Research_Page` and
 * `WP_MCP_AI_Admin_Team_Settings` preserve the public behaviour: the
 * byte-identical page slugs, the per-mode menu registration under the
 * team CPT menu, init/register idempotence (hook-registry dedup
 * delta), the per-mode post-type and shortcode seams, the research
 * enqueue gate + `wpMcpAiResearchPage` envelope (entityType=team), the
 * render surface (common surface incl. the View Professions
 * cross-link, no-assistant notice, import form, review quality
 * metrics with seeded teams), the four settings registrations, the
 * settings tab renders (overview/configuration/tools/help), the
 * capability gate, and the nonce-gated save flow (options +
 * temperature clamp + settings-updated redirect). Runs in both
 * matrices.
 *
...
use NvoosContentGraphAiPlatform\Admin\Integrations\TeamResearchPage;
use NvoosContentGraphAiPlatform\Admin\Integrations\TeamSettings;
 ...
 */
class TeamResearchPageExposer extends TeamResearchPage {

	public static function exposed_team_post_type(): string {
		return self::team_post_type();
	}
 ...
 */
class TeamSettingsExposer extends TeamSettings {
...
/**
 * Team pages characterisation suite (Wave E-UI-3, sub-cluster 2).
 */
#[\PHPUnit\Framework\Attributes\Group( 'integrations' )]
class Test_Team_Pages extends \WP_UnitTestCase {

	public function setUp(): void {
		parent::setUp();

		\wp_set_current_user( 0 );
		unset( $_GET['tab'], $_GET['settings-updated'], $_POST['wp_mcp_ai_team_settings_nonce'], $_POST['wp_mcp_ai_team_default_provider'], $_POST['wp_mcp_ai_team_default_model'], $_POST['wp_mcp_ai_team_default_temperature'] );

		\delete_option( 'wp_mcp_ai_team_settings' );
		\delete_option( 'wp_mcp_ai_team_default_provider' );
		\delete_option( 'wp_mcp_ai_team_default_model' );
		\delete_option( 'wp_mcp_ai_team_default_temperature' );
	...
	/**
	 * Create a published team post with optional meta.
	 *
	 * @param array $meta Meta key/value pairs.
	 * @return int
	 */
	private function create_team( array $meta = array() ): int {
		$id = self::factory()->post->create(
			array(
				'post_type'   => 'mcp_ai_team',
				'post_title'  => 'Seeded Team',
				'post_status' => 'publish',
			)
		);
	...
	public function test_research_page_slug_byte_identical(): void {
		$this->assertSame( 'research-team', TeamResearchPage::PAGE_SLUG );
	}
	...
	public function test_research_post_type_seam(): void {
		$this->assertSame( 'mcp_ai_team', TeamResearchPageExposer::exposed_team_post_type() );
	}

	public function test_research_shortcode_seam_resolves_per_install_mode(): void {
		if ( defined( 'WP_MCP_AI_PATH' ) ) {
			$this->assertSame( 'WP_MCP_AI_Shortcode', TeamResearchPageExposer::exposed_shortcode_class() );
		} else {
			$this->assertNull( TeamResearchPageExposer::exposed_shortcode_class() );
		}
	}
	...
	public function test_research_enqueue_assets_per_install_mode(): void {
		...
		TeamResearchPage::enqueue_assets( 'mcp_ai_team_page_research-team' );
		...
		// The localized envelope carries the byte-identical key + entityType.
		global $wp_scripts;
		...
		$this->assertStringContainsString( '"entityType":"team"', $data );
	...
	public function test_research_enqueue_skips_other_pages(): void {
		...
		TeamResearchPage::enqueue_assets( 'toplevel_page_something-else' );
	...
	public function test_research_render_common_surface(): void {
		$this->admin_user();
		$output = TeamResearchPageExposer::exposed_render();

		$this->assertStringContainsString( 'Research &amp; Add Team', $output );
		$this->assertStringContainsString( 'How It Works', $output );
		$this->assertStringContainsString( 'Choose Your Workflow', $output );
		$this->assertStringContainsString( 'Import Team Data', $output );
		...
		$this->assertStringContainsString( 'Team Data Quality', $output );
		$this->assertStringContainsString( 'View Professions', $output );
	...
	public function test_research_render_review_metrics(): void {
		// One complete + one partial team → completeness derived from the
		// byte-identical rules (members must be a non-empty array).
		$this->create_team(
			array(
				'_wp_mcp_ai_team_members'            => array( 1, 2 ),
				'_wp_mcp_ai_team_orchestration_mode' => 'sequential',
				'_wp_mcp_ai_team_driver_assistant_id' => 7,
			)
		);
		$this->create_team(
			array(
				'_wp_mcp_ai_team_members' => array( 3 ),
			)
		);
		...
		// the install may carry seeded teams beyond this test's two.
		$counts          = \wp_count_posts( 'mcp_ai_team' );
		...
		foreach ( $teams as $team ) {
			...
			if ( ! empty( $members ) && \is_array( $members ) && ! empty( $orchestration ) && ! empty( $driver ) ) {
				++$complete_count;
			}
		}
	...
	public function test_settings_page_slug_byte_identical(): void {
		$this->assertSame( 'wp-mcp-ai-team-settings', TeamSettings::PAGE_SLUG );
	}
	...
	public function test_settings_menu_registration_resolves_per_install_mode(): void {
		...
		if ( defined( 'WP_MCP_AI_PATH' ) ) {
			// Monolith: the base class owns the same page.
			( new \WP_MCP_AI_Admin_Team_Settings() )->register_submenu_page();
		} else {
			$page = new TeamSettings();
		...
		$this->assertArrayHasKey( $parent, $submenu );
		$slugs = \wp_list_pluck( $submenu[ $parent ], 2 );
		$this->assertContains( 'wp-mcp-ai-team-settings', $slugs );
	}
	...
	public function test_settings_post_type_seam(): void {
		$this->assertSame( 'mcp_ai_team', TeamSettingsExposer::exposed_team_post_type() );
	}
	...
	public function test_settings_available_providers_seam_resolves_per_install_mode(): void {
		if ( defined( 'WP_MCP_AI_PATH' ) ) {
			$this->assertSame(
				...
				TeamSettingsExposer::exposed_available_providers()
			);
		} else {
			$this->assertSame( array(), TeamSettingsExposer::exposed_available_providers() );
		}
	}
	...
	public function test_settings_register_settings(): void {
		...
		$this->assertArrayHasKey( 'wp_mcp_ai_team_default_provider', $wp_registered_settings );
		$this->assertArrayHasKey( 'wp_mcp_ai_team_default_model', $wp_registered_settings );
		$this->assertArrayHasKey( 'wp_mcp_ai_team_default_temperature', $wp_registered_settings );
		$this->assertArrayHasKey( 'wp_mcp_ai_team_default_driver_assistant', $wp_registered_settings );
	}
	...
	public function test_settings_render_blocks_non_managers(): void {
		...
		$message = $this->capture_die_message(
			static function (): void {
				( new TeamSettingsExposer() )->exposed_render();
			}
		);
	...
	public function test_settings_render_overview_tab(): void {
		$this->admin_user();
		$output = ( new TeamSettingsExposer() )->exposed_render();

		$this->assertStringContainsString( 'Team Settings', $output );
		$this->assertStringContainsString( 'team-settings-nav', $output );
		$this->assertStringContainsString( 'AI Teams Overview', $output );
		$this->assertStringContainsString( 'Multi-Agent Coordination', $output );
	...
	public function test_settings_render_configuration_tab(): void {
		...
		$this->assertStringNotContainsString( 'AI Teams Overview', $output );
	}
	...
	public function test_settings_render_tools_tab(): void {
		...
		$this->assertStringContainsString( 'Team Orchestration', $output );
	}
	...
	public function test_settings_render_help_tab(): void {
		...
		$this->assertStringContainsString( 'Create New Team', $output );
	}
	...
	public function test_settings_save_flow(): void {
		...
		$_POST['wp_mcp_ai_team_settings_nonce']    = \wp_create_nonce( 'wp_mcp_ai_team_settings' );
		$_POST['wp_mcp_ai_team_default_provider']  = 'openai';
		$_POST['wp_mcp_ai_team_default_model']     = 'gpt-4.1';
		$_POST['wp_mcp_ai_team_default_temperature'] = '2.5';
		...
		$this->assertStringContainsString( 'page=wp-mcp-ai-team-settings', $redirected );

		$this->assertSame( 'openai', \get_option( 'wp_mcp_ai_team_default_provider' ) );
		$this->assertSame( 'gpt-4.1', \get_option( 'wp_mcp_ai_team_default_model' ) );
		// The temperature is clamped to the 0–1 range (int coercion quirk).
		$this->assertEquals( 1.0, \get_option( 'wp_mcp_ai_team_default_temperature' ) );
	}
	...
	public function test_settings_render_ignores_invalid_nonce(): void {
		...
		$_POST['wp_mcp_ai_team_settings_nonce']   = 'bogus';
		$_POST['wp_mcp_ai_team_default_provider'] = 'openai';
		// phpcs:enable WordPress.Security.NonceVerification
		...
		$this->assertFalse( \get_option( 'wp_mcp_ai_team_default_provider' ) );
	}

	public function test_settings_tools_list_shape(): void {
		$page  = new TeamSettingsExposer();
